    ...
    reader.readAsDataURL(file);
        ...
        function validateAndSubmit(event) {
            ...
            comboRows.forEach((row, index) => {
                const size = row.querySelector(`input[name="combos[${index}].size"]`).value.trim();
                const quantity = row.querySelector(`input[name="combos[${index}].quantity"]`).value.trim();
                const regularPrice = row.querySelector(`input[name="combos[${index}].regularPrice"]`).value.trim();
                const salesPrice = row.querySelector(`input[name="combos[${index}].salesPrice"]`).value.trim();
                const color = row.querySelector(`input[name="combos[${index}].color"]`).value.trim();
        ...
        function validateForm() {
            ...
            combos.forEach((combo, index) => {
                const size = combo.querySelector(`input[name="combos[${index}].size"]`).value.trim();
                const quantity = combo.querySelector(`input[name="combos[${index}].quantity"]`).value.trim();
                const regularPrice = combo.querySelector(`input[name="combos[${index}].regularPrice"]`).value.trim();
                const salesPrice = combo.querySelector(`input[name="combos[${index}].salesPrice"]`).value.trim();
                const color = combo.querySelector(`input[name="combos[${index}].color"]`).value.trim();
                ...
                if (!/^\d+(\.\d{1,2})?$/.test(salesPrice) || parseFloat(salesPrice) < 0 || parseFloat(salesPrice) >= parseFloat(regularPrice)) {
                    displayErrorMessage('salesPrice-error', 'Please enter a valid non-negative sale price lower than the original price.');
                    isValid = false;
                }
